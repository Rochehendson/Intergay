...
/datum/mind/Topic(href, href_list)
	...
	if(href_list["add_goal"])
		...
		var/mob/callerr = locate(href_list["add_goal_caller"])
		if(!isghost(usr) && callerr && callerr == current) can_modify = TRUE
	...
	if(href_list["abandon_goal"])
		...
		var/mob/callerr = locate(href_list["abandon_goal_caller"])
		if(!isghost(usr) && callerr && callerr == current) can_modify = TRUE
	...
	if(href_list["reroll_goal"])
		...
		var/mob/callerr = locate(href_list["reroll_goal_caller"])
		if(!isghost(usr) && callerr && callerr == current) can_modify = TRUE
