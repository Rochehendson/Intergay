...
//Regular jobs
	//Command (Blue)
		...
		for(var/jobPos in SSjobs.titles_by_department(COM))
			...
			if(jobban_isbanned(M, job.title))
				jobs += "<td width='20%'><a href='byond://?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'><font color=red>[replacetext_char(job.title, " ", "&nbsp")]</font></a></td>"
				counter++
			else
				jobs += "<td width='20%'><a href='byond://?src=\ref[src];jobban3=[job.title];jobban4=\ref[M]'>[replacetext_char(job.title, " ", "&nbsp")]</a></td>"
				counter++
	...
	//Antagonist (Orange)
		...
		for(var/antag_type in all_antag_types)
			...
			if(jobban_isbanned(M, "[antag.id]"))
				jobs += "<td width='20%'><a href='byond://?src=\ref[src];jobban3=[antag.id];jobban4=\ref[M]'><font color=red>[replacetext_char("[antag.role_text]", " ", "&nbsp")]</font></a></td>"
			else
				jobs += "<td width='20%'><a href='byond://?src=\ref[src];jobban3=[antag.id];jobban4=\ref[M]'>[replacetext_char("[antag.role_text]", " ", "&nbsp")]</a></td>"
	...
	// Channels
		...
		for(var/channel_type in channels)
			...
			if(jobban_isbanned(M, channel.name))
				jobs += "<td width='20%'><a href='byond://?src=\ref[src];jobban3=[channel.name];jobban4=\ref[M]'><font color=red>[channel.name]</font></a></td>"
			else
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		for(var/datum/round_aspect/aspect as anything in typesof(/datum/round_aspect))
			dat += {"<a href='byond://?src=\ref[src];c_aspect2=[aspect]'>[initial(aspect.name)]</A><br>"}
