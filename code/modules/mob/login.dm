	...
	if(config.log_access)
		...
		for(var/mob/M in GLOB.player_list)
			...
			if( M.key && (M.key != key) )
				...
				if(matches)
					if(M.client)
						message_admins("[SPAN_DANGER("<B>Notice:</B>")] <span class='info'><a href='byond://?src=\ref[usr];priv_msg=\ref[src]'>[key_name_admin(src)]</A> has the same [matches] as <a href='byond://?src=\ref[usr];priv_msg=\ref[M]'>[key_name_admin(M)]</A>.</span>", 1)
						log_access("Notice: [key_name(src)] has the same [matches] as [key_name(M)].")
					else
						message_admins("[SPAN_DANGER("<B>Notice:</B>")] <span class='info'><a href='byond://?src=\ref[usr];priv_msg=\ref[src]'>[key_name_admin(src)]</A> has the same [matches] as [key_name_admin(M)] (no longer logged in).</span>", 1)
